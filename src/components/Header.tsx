...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      {/* Top bar */}
      <div className="bg-[var(--primary-dark)] border-b border-[var(--border)]">
        <div className="container flex items-center justify-between py-2 text-sm">
          <div className="flex items-center gap-4">
            <Link href="/shop" className="text-[var(--text-secondary)] hover:text-[var(--secondary)] transition-colors">
              Shop
            </Link>
            <Link href="/kontakt" className="text-[var(--text-secondary)] hover:text-[var(--secondary)] transition-colors">
              Kontakt
            </Link>
          </div>
          <div className="flex items-center gap-4">
            <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[var(--secondary)]">
            ...
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[var(--secondary)]">
            ...
            <a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text-secondary)] hover:text-[var(--secondary)]">
        ...
        <div className="container flex items-center justify-between py-4">
          ...
          <Link href="/" className="flex items-center gap-3">
            <div className="w-12 h-12 bg-[var(--secondary)] rounded-full flex items-center justify-center">
              <span className="text-2xl font-bold text-[var(--primary-dark)]">柔</span>
            </div>
            <div className="hidden sm:block">
              <h1 className="text-xl font-bold text-white">Judo Klub</h1>
              <p className="text-xs text-[var(--text-secondary)]">Sarajevo</p>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {menuItems.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className="px-4 py-2 text-white hover:text-[var(--secondary)] transition-colors font-medium flex items-center gap-1"
                >
                  {item.label}
                  {item.submenu && (
                ...
                {item.submenu && activeSubmenu === item.label && (
                  <div className="absolute top-full left-0 mt-0 w-56 bg-[var(--surface)] rounded-lg shadow-xl border border-[var(--border)] overflow-hidden animate-fade-in-up">
                    {item.submenu.map((subItem) => (
                      <Link
                        key={subItem.label}
                        href={subItem.href}
                        className="block px-4 py-3 text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--secondary)] transition-colors border-b border-[var(--border)] last:border-b-0"
                      >
                        {subItem.label}
                      </Link>
                    ))}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-[var(--border)]">
            <div className="container py-4">
              ...
              <Link href="/clanstvo" className="btn-primary block text-center mt-4" onClick={() => setMobileMenuOpen(false)}>
                Postani član
              </Link>
            </div>
          </div>
