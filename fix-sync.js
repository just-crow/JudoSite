...
let content = fs.readFileSync('src/lib/competition-sync.ts', 'utf8');

const replacement = \`        try {
            /* If past competition, fetch and sync medals to existing active competitors */
            if (new Date(comp.dateTo) < new Date()) {
                await syncCompetitorMedals(comp);
            }

            /* Upsert: match on title + date (unique enough for judo comps) */\`;

content = content.replace(/\\s*try \\{\\s*\\/\\* Upsert: match on title \\+ date \\(unique enough for judo comps\\) \\*\\//, replacement);
...
fs.writeFileSync('src/lib/competition-sync.ts', content, 'utf8');
